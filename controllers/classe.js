module.exports = function (app, queryPromise) {
    ...
    app.post('/index/classe', async (req, res) => {
        ...
        try {
            ...
            if (id_utilisateur == "") {
                return res.status(404).json({ error: 'Selectionnez votre pseudo !' });
            }

            else if (nb_id != parseInt(nb_id) && nb_id.count == 0) {
        ...
        if (niveau != parseInt(niveau) || niveau == "") {
            return res.status(404).json({ error: 'Selectionnez votre niveau !' });
        }
        else if (niveau <= 0 || niveau > 100) {
            return res.status(404).json({ error: 'Les niveaux sont compris de 1 jusque 100.' });
        }
